...
RmScan::RmScan(const RmFileHandle *file_handle) : file_handle_(file_handle), rid_{RM_FIRST_RECORD_PAGE, -1} {
    next();
}

void RmScan::next() {
    while (rid_.page_no != RM_NO_PAGE && rid_.page_no < file_handle_->file_hdr_.num_pages) {
