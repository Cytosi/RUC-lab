...
class AbstractExecutor {
   public:
    ...
    static bool CompareforOp(CompOp op, int cmp) {
        if (op == OP_EQ) {
            return cmp == 0;
        } else if (op == OP_NE) {
            return cmp != 0;
    ...
    bool CheckCond(const RmRecord *l_record, const std::vector<Condition>& conds_, const std::vector<ColMeta>& cols_) {
